...
@everywhere fitness=Dict([0,0,0]=>1, 
            [1,0,0]=>1.3,
            [0,1,0]=>1.2,
            [1,1,0]=>1.5,
            [1,1,1]=>1.2)
...
@everywhere scenario_0D = create_scenario((1000000),10)
@everywhere scenario_1D = create_scenario((1000000,),10,"center")
@everywhere scenario_2D = create_scenario((1000,1000),10,"center")
@everywhere scenario_3D = create_scenario((100,100,100),10,"center")
...
@everywhere adaptive_therapy = Treatment(3000, 2000, 1000, 3, 0.75, false, false) 

@everywhere continuous_therapy = Treatment(3000, 2000, 0, 3, 0.75, false, false) 

#We define what we want to collect
@everywhere agent_collect = [(:genotype, f) for f in genotype_fraction_function_generator(fitness)]
...
@everywhere steps=1
adata, _ = paramscan(parameters, model_init; adata = agent_collect, agent_step! = agent_step!, model_step! = model_step!, n = steps, parallel = true, showprogress = true,include_constants=true)
